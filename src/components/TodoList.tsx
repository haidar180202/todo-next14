...
const TodoList: React.FC = () => {
  ...
  const handleKeyUpdate = (event: any, id: number) => {
    if (event.key === 'Enter' && event.target.value >= 1 || (event.type === 'blur' &&  event.target.value >= 1)) {
      let weightInput = event.target.value;
      dispatch(setMoveUpdateData({ id: id, jumlah: weightInput }));
  ...
  const handleButtonClick = (index:any) => { // fungsi untuk setting perubahan status saat card bahan diklik maka akan menampilkan buttonnya lalu menghide berdasarkan waktu tertentu secara otomatis saat tidak di klik
...
const handleForSettingWaktuJumlah  = (index:any) => { // fungsi untuk setting perubahan status saat card bahan diklik maka akan menampilkan buttonnya lalu menghide berdasarkan waktu tertentu secara otomatis saat tidak di klik
  ...
  const handleForSettingWaktuTatacara  = (index:any) => { // fungsi untuk setting perubahan status saat card bahan diklik maka akan menampilkan buttonnya lalu menghide berdasarkan waktu tertentu secara otomatis saat tidak di klik
  ...
  const handleUpdateJumlah  = (event:any,id:any) => { // fungsi untuk setting perubahan status saat card bahan diklik maka akan menampilkan buttonnya lalu menghide berdasarkan waktu tertentu secara otomatis saat tidak di klik
    // if(event.key === 'Enter' && event.target.value !== "" || event.type === 'blur' &&  event.target.value >= 1){
    //  let weightInput = event.target.value;
    //  console.log(weightInput);
    //   //  dispatch(setMoveUpdateData({ id: id, keterangan: weightInput }));
    //   //  dispatch(setManipulateJumlah(false));
    // }
    console.log(event,id,"teken trus");
      ...
      <CardBody>
          ...
          <button key={index} onClick={() => handleButtonClick(index)}>
            <div className="flex mb-2 justify-between" key={todo.id}>
              <div>
                ...
                <p>
                  {todo.jumlah === 0  ? (<input type="number" min={1} className="mx-2 w-10 py-2 mt-1" style={{ textAlign: 'center' }} onKeyDown={(event) => handleKeyDown(event, todo.id)} onBlur={(event) => handleKeyDown(event, todo.id)} defaultValue={todo.jumlah >= 1 ? todo.jumlah : ''}/>) :  StatusKlikManipulasiJumlahBahanResep.status === true && clickedIndexRefForEditJumlah.current === index ? (<input type="number" min={1} className="mx-2 w-10 py-2 mt-1" style={{ textAlign: 'center' }} onKeyDown={(event) => handleKeyUpdate(event, todo.id)} onBlur={(event) => handleKeyUpdate(event, todo.id)} defaultValue={todo.jumlah >= 1 ? todo.jumlah : ''}/>) :(<>{todo.jumlah}</>) }{' '} kg <span className='ml-3'>{todo.name}</span>
                </p>
              </div>
              ...
              <div className="flex space-x-1">
                <div className="my-auto">
                  <BsPencil
                    ...
                    onClick={() => handleForSettingWaktuJumlah(index)}
        ...
        <div>
          ...
          <div className="my-auto">
              ...
              <div key={dd.id}>
                {StatusKlikManipulasiTatacara.status === true && clickedIndexRefForEditTatacara.current === index ? (  
                 ...
                 />
                  ...
                  <button onClick={()=>handleForSettingWaktuTatacara(index)}>
            ...
            <input type={moveLocationTodo.length === 0 ? 'disabled' : 'text'} className="mx-2 py-1 mt-1" disabled={moveLocationTodo.length === 0} onKeyDown={(event) => handleAddKeterangan(event)}  onBlur={(event) => handleAddKeterangan(event)}/>
          </div>
        </div>
